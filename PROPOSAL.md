...
The user will be able to give instructions and view the process on the master computer, and also open other computers to be used as nodes.
...
Output will be stored in files. 
It is especially important to make use of semaphores for distributed workloads that will have multiple outputs written to one file.
...
Jan 8, Thursday
Make the master send execution instructions to multiple nodes, and their outputs to flow to a file in the correct order using semaphores.
...
Master is able to see and kill processes on nodes.
